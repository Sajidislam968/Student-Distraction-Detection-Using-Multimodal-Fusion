...
# YOLO Settings
# =====================================================
# Your custom data.yaml class order: 0=bottle, 1=cup, 2=book
# This version can use BOTH models together:
# 1) Pretrained YOLOv8n for phone detection
# 2) Your custom trained model for bottle/cup/book detection

ENABLE_YOLO = True

USE_PRETRAINED_YOLO = True
PRETRAINED_YOLO_MODEL_PATH = "yolov8n.pt"

USE_CUSTOM_YOLO = True
CUSTOM_YOLO_MODEL_PATH = "custom_yolo_runs/focus_objects_fast/weights/best.pt"
CUSTOM_YOLO_FALLBACK_PATHS = [
    "custom_yolo_runs/focus_objects_fast/weights/best.pt",
    "custom_yolo_runs/focus_objects_fast/weights/last.pt",
    "runs/focus_objects_yolov8n_fast/weights/best.pt",
    "runs/focus_objects_yolov8n_fast/weights/last.pt",
    "runs/detect/runs/detect/focus_objects_yolov8n/weights/best.pt",
    "runs/detect/runs/detect/focus_objects_yolov8n/weights/last.pt",
    "runs/detect/focus_objects_yolov8n/weights/best.pt",
    "runs/detect/focus_objects_yolov8n/weights/last.pt",
]

# Detection speed settings for CPU laptop
YOLO_IMAGE_SIZE = 416
YOLO_CONFIDENCE = 0.35
YOLO_DETECT_EVERY_N_FRAMES = 5

# Backward-compatible variable used only for old comments/tools
YOLO_MODEL_PATH = PRETRAINED_YOLO_MODEL_PATH
...
# Personalized behavior tracking and adaptation files.
PERSONALIZATION_DIR = os.path.join(BASE_DIR, "personalization")
USER_PROFILE_PATH = os.path.join(PERSONALIZATION_DIR, "user_profile.json")
PERSONAL_ACTIVITY_LOG_PATH = os.path.join(PERSONALIZATION_DIR, "personal_activity_log.csv")
...
# =====================================================
# Personalization / Adaptation Settings
# =====================================================

ENABLE_PERSONALIZATION = True

# How often the system saves personalized behavior samples.
PERSONAL_ACTIVITY_LOG_INTERVAL = 5.0

# How often the personal profile is recalculated from saved user behavior.
PERSONAL_PROFILE_UPDATE_INTERVAL = 30.0

# Personalization never goes beyond these safe ranges.
MIN_ADAPTIVE_ALERT_SECONDS = 5.0
MAX_ADAPTIVE_ALERT_SECONDS = 20.0
MIN_ADAPTIVE_LONG_IDLE_SECONDS = 45.0
MAX_ADAPTIVE_LONG_IDLE_SECONDS = 180.0
MIN_ADAPTIVE_APP_CONFIDENCE = 60.0
MAX_ADAPTIVE_APP_CONFIDENCE = 80.0
...
class VisionWorker(threading.Thread):
    def __init__(self, shared_state, stop_event):
        ...
        # YOLO models
        self.pretrained_yolo_model = None
        self.pretrained_yolo_names = {}
        self.pretrained_phone_class_ids = None

        self.custom_yolo_model = None
        self.custom_yolo_names = {}
        self.custom_object_class_ids = None

        # Run YOLO every few frames to keep the full system faster on CPU
        self.yolo_frame_count = 0
        self.cached_phone_detected = 0
        self.cached_object_detected = 0
        self.cached_detected_objects = ""
    ...
    def find_existing_yolo_path(self, preferred_path, fallback_paths):
        """
        Finds the first existing YOLO model path.
        This allows the system to use best.pt from either your fast training
        or the long training that you may stop early.
        """
        candidates = []
    ...
    def get_class_ids_by_name(self, names_dict, target_names):
        """
        Returns YOLO class IDs matching target class names.
        Safer than hardcoding COCO class IDs.
        """
        class_ids = []
    ...
    def load_yolo_if_enabled(self):
        ...
        try:
            ...
            self.shared_state.update(
                "yolo",
                yolo_status=f"YOLO Import Error: {e}",
                phone_detected=0,
                object_detected=0,
                detected_objects=""
            )
        ...
        # ---------------------------------------------
        # 1) Pretrained YOLO for phone detection
        # ---------------------------------------------
        if USE_PRETRAINED_YOLO:
            try:
                self.pretrained_yolo_model = YOLO(PRETRAINED_YOLO_MODEL_PATH)
                self.pretrained_yolo_names = self.pretrained_yolo_model.names

                self.pretrained_phone_class_ids = self.get_class_ids_by_name(
                    self.pretrained_yolo_names,
                    {"cell phone", "phone", "mobile phone"}
                )

                loaded_parts.append("pretrained phone model")

            except Exception as e:
                self.pretrained_yolo_model = None
                errors.append(f"pretrained error: {e}")

        # ---------------------------------------------
        # 2) Custom YOLO for book/cup/bottle detection
        # ---------------------------------------------
        if USE_CUSTOM_YOLO:
            custom_path = self.find_existing_yolo_path(
            ...
            if custom_path is not None:
                try:
                    self.custom_yolo_model = YOLO(custom_path)
                    self.custom_yolo_names = self.custom_yolo_model.names

                    self.custom_object_class_ids = self.get_class_ids_by_name(
                        self.custom_yolo_names,
                        {"book", "cup", "bottle", "water bottle"}
                    )

                    loaded_parts.append(f"custom object model: {custom_path}")

                except Exception as e:
                    self.custom_yolo_model = None
                    errors.append(f"custom error: {e}")
            else:
                errors.append("custom model not found; using pretrained phone only")

        if self.pretrained_yolo_model is None and self.custom_yolo_model is None:
            self.shared_state.update(
                "yolo",
                yolo_status="No YOLO model loaded: " + "; ".join(errors),
                phone_detected=0,
                object_detected=0,
                detected_objects=""
            )
        ...
        status = "Loaded: " + " + ".join(loaded_parts)
        if errors:
            status += " | " + " | ".join(errors)
    ...
    def draw_yolo_box(self, frame, box, label):
    ...
    def run_yolo(self, frame):
        ...
        # Between detections, keep the last detected status.
        self.yolo_frame_count += 1

        if self.yolo_frame_count % YOLO_DETECT_EVERY_N_FRAMES != 0:
            return (
                frame,
                self.cached_phone_detected,
                self.cached_object_detected,
                self.cached_detected_objects
            )
        ...
        # ---------------------------------------------
        # Pretrained model: detect phone only
        # ---------------------------------------------
        try:
            if self.pretrained_yolo_model is not None:
                kwargs = {
                ...
                if self.pretrained_phone_class_ids:
                    kwargs["classes"] = self.pretrained_phone_class_ids

                results = self.pretrained_yolo_model(frame, **kwargs)

                for result in results:
                    for box in result.boxes:
                        cls_id = int(box.cls[0])
                        class_name = str(self.pretrained_yolo_names[cls_id]).lower()
        ...
        # ---------------------------------------------
        # Custom model: detect book/cup/bottle
        # ---------------------------------------------
        try:
            if self.custom_yolo_model is not None:
                kwargs = {
                    "conf": YOLO_CONFIDENCE,
                    "imgsz": YOLO_IMAGE_SIZE,
                    "verbose": False,
                ...
                if self.custom_object_class_ids:
                    kwargs["classes"] = self.custom_object_class_ids

                results = self.custom_yolo_model(frame, **kwargs)

                for result in results:
                    for box in result.boxes:
                        cls_id = int(box.cls[0])
                        class_name = str(self.custom_yolo_names[cls_id]).lower()
                        confidence = float(box.conf[0])

                        if class_name == "water bottle":
                            class_name = "bottle"

                        if class_name in {"book", "cup", "bottle"}:
                            object_detected = 1
        ...
        self.cached_phone_detected = phone_detected
        self.cached_object_detected = object_detected
        self.cached_detected_objects = detected_objects_text
...
class AutoLabelWorker(threading.Thread):
    ...
    def decide_label(self, snapshot):
        ...
        adaptive_app_confidence = float(
            snapshot.get(
                "adaptive_focused_app_confidence",
                HYBRID_FOCUSED_APP_CONFIDENCE
            )
        )

        adaptive_long_idle = float(
            snapshot.get(
                "adaptive_long_idle_seconds",
                HYBRID_LONG_IDLE_SECONDS
            )
        )

        focused_app_condition = (
            app_label == "Focused"
            and app_confidence >= adaptive_app_confidence
        )
        ...
        long_idle = mouse_idle_time >= adaptive_long_idle
...
class FusionDecisionWorker(threading.Thread):
    ...
    def has_strong_focused_evidence(self, snapshot):
        ...
        not_long_idle = mouse_idle_time < adaptive_long_idle
    ...
    def hybrid_decision(self, raw_prediction, confidence, probabilities, snapshot):
        ...
        adaptive_xgboost_threshold = float(
            snapshot.get(
                "adaptive_xgboost_trust_threshold",
                HYBRID_XGBOOST_TRUST_THRESHOLD
            )
        )

        xgboost_is_high_confidence = (
            confidence >= adaptive_xgboost_threshold
        )
        ...
        if xgboost_is_high_confidence:
            return (
                ...
                (
                    ...
                    f"({confidence:.2f}%, threshold {adaptive_xgboost_threshold:.2f}%)"
                )
            )
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            if final_decision == "Distracted":
                ...
                adaptive_alert_seconds = float(
                    snapshot.get(
                        "adaptive_alert_seconds",
                        DISTRACTION_ALERT_SECONDS
                    )
                )

                should_alert = (
                    distracted_duration >= adaptive_alert_seconds
                ...
                if should_alert:
                    ...
                    self.shared_state.update(
                        ...
                        alert_message=(
                            ...
                            f"alert threshold {adaptive_alert_seconds:.0f} sec"
                        ),
                        alert_count=self.alert_count,
                        distracted_alert_duration=round(distracted_duration, 2)
                    )
...
# =====================================================
# Personalized Behavior Tracking / Adaptation Worker
# =====================================================

class PersonalizationWorker(threading.Thread):
    def __init__(self, shared_state, stop_event):
        super().__init__(daemon=True)
        self.shared_state = shared_state
        self.stop_event = stop_event
        ...
        self.publish_profile_to_shared_state()
    ...
    def default_profile(self):
        return {
            ...
            "personal_focus_rate": 0.0,
            "personal_distracted_rate": 0.0,
            "personal_neutral_rate": 0.0,
            "personal_absent_rate": 0.0,
            "top_focused_app": "Unknown",
            "top_distracting_app": "Unknown",
            "top_distraction_reason": "Unknown",
            "adaptive_xgboost_trust_threshold": HYBRID_XGBOOST_TRUST_THRESHOLD,
            "adaptive_focused_app_confidence": HYBRID_FOCUSED_APP_CONFIDENCE,
            "adaptive_long_idle_seconds": HYBRID_LONG_IDLE_SECONDS,
            "adaptive_alert_seconds": DISTRACTION_ALERT_SECONDS,
            "adaptation_note": "Default profile; learning from this user's behavior.",
        }
    ...
    def publish_profile_to_shared_state(self):
        self.shared_state.update(
            "personalization",
            personalization_enabled=1 if ENABLE_PERSONALIZATION else 0,
            personal_profile_path=USER_PROFILE_PATH,
            personal_activity_log_path=PERSONAL_ACTIVITY_LOG_PATH,
            personal_focus_rate=self.profile.get("personal_focus_rate", 0.0),
            personal_distracted_rate=self.profile.get("personal_distracted_rate", 0.0),
            personal_neutral_rate=self.profile.get("personal_neutral_rate", 0.0),
            personal_absent_rate=self.profile.get("personal_absent_rate", 0.0),
            total_behavior_records=self.profile.get("total_behavior_records", 0),
            top_focused_app=self.profile.get("top_focused_app", "Unknown"),
            top_distracting_app=self.profile.get("top_distracting_app", "Unknown"),
            top_distraction_reason=self.profile.get("top_distraction_reason", "Unknown"),
            adaptive_xgboost_trust_threshold=self.profile.get(
                "adaptive_xgboost_trust_threshold",
                HYBRID_XGBOOST_TRUST_THRESHOLD
            ),
            adaptive_focused_app_confidence=self.profile.get(
                "adaptive_focused_app_confidence",
                HYBRID_FOCUSED_APP_CONFIDENCE
            ),
            adaptive_long_idle_seconds=self.profile.get(
                "adaptive_long_idle_seconds",
                HYBRID_LONG_IDLE_SECONDS
            ),
            adaptive_alert_seconds=self.profile.get(
                "adaptive_alert_seconds",
                DISTRACTION_ALERT_SECONDS
            ),
            adaptation_note=self.profile.get(
                "adaptation_note",
                "Default profile; learning from this user's behavior."
            )
        )

    def ensure_activity_log(self):
        if not os.path.exists(PERSONAL_ACTIVITY_LOG_PATH):
            with open(
                PERSONAL_ACTIVITY_LOG_PATH,
                "w",
                newline="",
                encoding="utf-8"
            ) as f:
    ...
    def read_activity_rows(self):
        ...
        try:
            ...
            # Keep profile calculation fast by using recent records.
            return rows[-1000:]
    ...
    def most_common(self, values, default="Unknown"):
        counts = {}
    ...
    def percentile(self, values, percentile_value, default):
    ...
    def update_profile_from_activity(self):
        rows = self.read_activity_rows()
        ...
        for row in rows:
            state = str(row.get("final_decision", "Neutral")).strip()
        ...
        focus_rate = round((state_counts["Focused"] / total) * 100.0, 2)
        distracted_rate = round((state_counts["Distracted"] / total) * 100.0, 2)
        neutral_rate = round((state_counts["Neutral"] / total) * 100.0, 2)
        absent_rate = round((state_counts["Absent"] / total) * 100.0, 2)

        focused_rows = [
            row for row in rows
            if str(row.get("final_decision", "")) == "Focused"
        ]

        distracted_rows = [
            row for row in rows
            if str(row.get("final_decision", "")) == "Distracted"
        ]

        top_focused_app = self.most_common(
            [
                row.get("app_title", "")
                for row in focused_rows
                if row.get("app_title", "")
            ],
            default="Unknown"
        )

        top_distracting_app = self.most_common(
            [
                row.get("app_title", "")
                for row in distracted_rows
                if row.get("app_title", "")
            ],
            default="Unknown"
        )

        top_distraction_reason = self.most_common(
            [
                row.get("auto_reason", "")
                for row in distracted_rows
                if row.get("auto_reason", "")
            ],
            default="Unknown"
        )

        focused_idle_values = [
            self.safe_float(row.get("mouse_idle_time", 0.0))
            for row in focused_rows
        ]

        focused_app_confidences = [
            self.safe_float(row.get("app_confidence", 0.0))
            for row in focused_rows
            if str(row.get("app_label", "")) == "Focused"
        ]

        # Adapt long-idle threshold to the user's real focused behavior.
        # Example: reading slides/PDF may produce long mouse idle periods.
        if len(focused_idle_values) >= 5:
            p90_idle = self.percentile(
                focused_idle_values,
                90,
                HYBRID_LONG_IDLE_SECONDS
            )

            adaptive_long_idle = self.clamp(
                p90_idle + 10.0,
                MIN_ADAPTIVE_LONG_IDLE_SECONDS,
                MAX_ADAPTIVE_LONG_IDLE_SECONDS
            )
        else:
            adaptive_long_idle = self.profile.get(
                "adaptive_long_idle_seconds",
                HYBRID_LONG_IDLE_SECONDS
            )

        # Adapt focused-app confidence threshold.
        # If this user's valid focused apps are often around 60-70%,
        # relax it slightly, but keep safe bounds.
        if len(focused_app_confidences) >= 5:
            p25_confidence = self.percentile(
                focused_app_confidences,
                25,
                HYBRID_FOCUSED_APP_CONFIDENCE
            )

            adaptive_app_confidence = self.clamp(
                p25_confidence - 2.0,
                MIN_ADAPTIVE_APP_CONFIDENCE,
                MAX_ADAPTIVE_APP_CONFIDENCE
            )
        else:
            adaptive_app_confidence = self.profile.get(
                "adaptive_focused_app_confidence",
                HYBRID_FOCUSED_APP_CONFIDENCE
            )

        # Adapt alert threshold based on this user's distraction pattern.
        # More distracted sessions -> faster alerts. More focused sessions -> less aggressive alerts.
        if distracted_rate >= 40.0:
            adaptive_alert_seconds = 8.0
        elif focus_rate >= 70.0:
            adaptive_alert_seconds = 12.0
        else:
            adaptive_alert_seconds = DISTRACTION_ALERT_SECONDS

        adaptive_alert_seconds = self.clamp(
            adaptive_alert_seconds,
            MIN_ADAPTIVE_ALERT_SECONDS,
            MAX_ADAPTIVE_ALERT_SECONDS
        )

        # Keep model trust threshold safe but personalized.
        # If XGBoost has been stable/high-confidence for this user, trust it a bit more.
        # Otherwise keep the stricter default.
        xgb_confidences = [
            self.safe_float(row.get("fusion_confidence", 0.0))
            for row in rows
        ]

        avg_xgb_confidence = (
            sum(xgb_confidences) / len(xgb_confidences)
            if xgb_confidences
            else HYBRID_XGBOOST_TRUST_THRESHOLD
        )

        if avg_xgb_confidence >= 85.0:
            adaptive_xgboost_threshold = 78.0
        else:
            adaptive_xgboost_threshold = HYBRID_XGBOOST_TRUST_THRESHOLD

        self.profile.update({
            "profile_version": 1,
            "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "total_behavior_records": total,
            "personal_focus_rate": focus_rate,
            "personal_distracted_rate": distracted_rate,
            "personal_neutral_rate": neutral_rate,
            "personal_absent_rate": absent_rate,
            "top_focused_app": top_focused_app,
            "top_distracting_app": top_distracting_app,
            "top_distraction_reason": top_distraction_reason,
            "adaptive_xgboost_trust_threshold": round(adaptive_xgboost_threshold, 2),
            "adaptive_focused_app_confidence": round(adaptive_app_confidence, 2),
            "adaptive_long_idle_seconds": round(adaptive_long_idle, 2),
            "adaptive_alert_seconds": round(adaptive_alert_seconds, 2),
            "adaptation_note": (
                "Profile updated from recent personal behavior logs. "
                "Thresholds are adapted within safe limits."
            ),
        })
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if current_time - self.last_log_time >= PERSONAL_ACTIVITY_LOG_INTERVAL:
                    self.append_activity_row(snapshot)
                    self.last_log_time = current_time

                if current_time - self.last_profile_update_time >= PERSONAL_PROFILE_UPDATE_INTERVAL:
                    self.update_profile_from_activity()
                    self.last_profile_update_time = current_time
...
def print_dashboard(snapshot):
    ...
    print("-" * 80)
    print(f"Personal Focus Rate      : {snapshot.get('personal_focus_rate', 0.0)}%")
    print(f"Personal Distracted Rate : {snapshot.get('personal_distracted_rate', 0.0)}%")
    print(f"Top Focused App          : {snapshot.get('top_focused_app', 'Unknown')}")
    print(f"Top Distracting App      : {snapshot.get('top_distracting_app', 'Unknown')}")
    print(f"Adaptive XGB Threshold   : {snapshot.get('adaptive_xgboost_trust_threshold', HYBRID_XGBOOST_TRUST_THRESHOLD)}%")
    print(f"Adaptive App Confidence  : {snapshot.get('adaptive_focused_app_confidence', HYBRID_FOCUSED_APP_CONFIDENCE)}%")
    print(f"Adaptive Long Idle       : {snapshot.get('adaptive_long_idle_seconds', HYBRID_LONG_IDLE_SECONDS)} sec")
    print(f"Adaptive Alert Seconds   : {snapshot.get('adaptive_alert_seconds', DISTRACTION_ALERT_SECONDS)} sec")
...
def main():
    ...
    workers = [
        ...
        PersonalizationWorker(shared_state, stop_event),
